...
class ImageService
{
    ...
    public function getAdapter(Strategy $strategy): FilesystemAdapter
    {
        ...
        return match ($strategy->key) {
            ...
            StrategyKey::Obs => new ObsAdapter(new Client(([
                    'accessKey'    => $configs->get(ObsOption::AccessKey),
                    'secretKey' => $configs->get(ObsOption::SecretKey),
                    'endpoint' => $configs->get(ObsOption::Endpoint),
                    'region' => $configs->get(ObsOption::Region),
                    'bucket_endpoint' => (bool)$configs->get(ObsOption::BucketEndpoint),
                ])),
                bucket: $configs->get(ObsOption::Bucket),
            ),
        };
    }
